...
class SessionController < ApplicationController
  ...
  def index
    redirect_to(:action => 'signup') unless logged_in? || CetUser.count > 0
  end

  def login
    return unless request.post?
    self.current_cet_user = authenticate(params[:login], params[:password])
    if logged_in?
      if params[:remember_me] == "1"
        self.current_cet_user.remember_me
  ...
  def logout
    #self.current_cet_user.forget_me if logged_in?
    cookies.delete :auth_token
    reset_session
    flash[:notice] = "You have been logged out."
    redirect_back_or_default(:controller => '/session', :action => 'login')
  end
	...
	class Authenticator
		...
		def authenticate(login, password)
			db_user = CetUser.find_by_login(login)
			begin
				email = login + "@" + @domain
